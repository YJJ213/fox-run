"use strict";
/* 本文件=网页版 狐狸快跑.html 的 <script> 抠出副本,只作移植参考,不参与运行。
   每次网页版大改后由工具刷新;真源码永远以 ../狐狸快跑.html 为准。 */
...
function juiceVibrate(kind){
  try{ navigator.vibrate && navigator.vibrate(20); }catch(e){}   // 恢复原版：统一轻震（撤掉震动分级，避免撞击过重）
}
    ...
    for(const o of obstacles){
      ...
      for(const b of obstacleBoxes(o)){
        if(pb.x < b.x + b.w && pb.x + pb.w > b.x){
          const gap = Math.max(b.y - (pb.y + pb.h), pb.y - (b.y + b.h));
...
function drawHUD(){
  ...
  if(adventureMode && curStage){
    ...
    subLine = '☀️ 今日最佳 ' + drB + ' 分'; subColor = '#ffd34d';
  } else if(breaking){
...
function render(){
  ...
  if(save.runs <= 1 && game.state === 'playing' && !jumpedRun && !paused){
    ...
    ctx.strokeText('👆 点屏幕 / 空格 跳！', player.x + player.w / 2, hy);
    ctx.fillText('👆 点屏幕 / 空格 跳！', player.x + player.w / 2, hy);
    ctx.restore();
  }
  ctx.restore();
  drawHUD();
...
function renderHome(){
  ...
  // 【收集进度总览】把散在各页的收集度聚到主页一行：一眼看到"就差几个了" → 想再开一局补齐
  ...
  document.getElementById('homeStats').innerHTML =
    ...
    '<br>📖 ' + achN + '/' + ACHIEVEMENTS.length + '成就 · 🗺️ ' + starN + '★/36 · 🦊 ' + charN + '/' + Object.keys(CHARS).length + '角色' +
