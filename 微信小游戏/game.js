...
function speedRamp(){
  ...
  // 【可玩性】平滑连续加速：从 +4/秒（开局温和不吓新手）顺滑爬到 +15/秒（约3000米封顶），全程无台阶跳变，速度变化自然不突兀
  return 4 + 11 * Math.min(1, m / 3000);
}
...
// 【可玩性】把里程映射成连续的"丰富度档位"(0..4)，锚点 0/280/1100/2200/4500 米线性插值——配合下面 frac² 概率渐入，让障碍花样平滑变多，而不是到某米数整档突然全换
function tierFloat(d){
  const pts = [[0, 0], [280, 1], [1100, 2], [2200, 3], [4500, 4]];
  if(d >= 4500) return 4;
...
function spawnObstacle(){
  ...
  if(!patQueue.length){
    ...
    if(tier < 4 && srand() < (tf - tier) * (tf - tier)) tier++;   // 【可玩性】软过渡：越接近下一档边界，越常掺入高一档的段（frac² 让早期只偶尔预览、不突兀拔高），到点不再障碍整档突然全换/变难。档位锚点：280米滑铲/坑/蹦床、1100米火焰/锯/激光、2200米更密、4500米硬核
...
function render(){
  ...
  if(game.state === 'playing' && !paused){
    ...
    const speedK = clamp((spd - 500) / 380, 0, 1) + (power.type === 'dash' ? 0.55 : 0) + (bgTime < feverUntil ? 0.4 : 0);
