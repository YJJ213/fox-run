...
const SPEED_MAX   = 700;       // 【节奏大修】速度硬顶 820→700：旧顶速下障碍出现到撞脸只剩 1 秒，普通人反应不过来；700 只留给 12500 米后的高手余档
// 【节奏大修】速度档位表 [里程(米), 目标速度]：把旧的"连续暗涨、73秒焊死顶速"改成天天酷跑式档位平台——
// 每到一档，用固定 2.5 秒平滑爬到新速度（无瞬跳），然后停在平台期让玩家适应，档位间隔前密后疏。
// 前 9 档=正常局（6500米/约2分40秒到 660）；尾部 3 档=狂飙余档，只有长跑老玩家摸得到。调节奏改这张表即可。
const GEARS = [[0,230],[200,300],[500,360],[900,420],[1400,470],[2000,520],[2800,565],[3800,605],[5000,640],[6500,660],[8000,675],[10000,690],[12500,SPEED_MAX]];
function gearIdx(m){   // 当前里程（米）落在第几档（0 起）
  let gi = 0;
  while(gi + 1 < GEARS.length && m >= GEARS[gi + 1][0]) gi++;
  return gi;
}
...
const BOSS_SCORE = 10000;      // 【血条Boss】第一次 Boss 战门槛（【节奏大修】12000→10000：曲线放缓后同分数要跑更久，降门槛保住中期高潮节拍）；之后每 +15000 再来一只
...
// 【性能】低端安卓一进门就降级（benchmarkLevel 只在安卓有，0~50 越小越弱；iOS 拿不到走运行时自动降级）
try{ const bm = wx.getSystemInfoSync().benchmarkLevel; if(typeof bm === 'number' && bm >= 0 && bm <= 10) setTimeout(() => { lowFx = true; }, 0); }catch(e){}
...
// 【美术】ChatGPT 生成的标题艺术字 + 分享卡主视觉（包内素材；没加载出来时各处自动退回旧的代码画法）
const logoImg = wx.createImage(); logoImg.src = 'assets/art/logo-fox-run.png';
const shareArtImg = wx.createImage(); shareArtImg.src = 'assets/art/share-key-art.jpg';
...
wx.onHide(function(){
  ...
  // 【长按蹲·审查修正】切后台会吞掉抬起事件：一律视为全松开，回来不卡蹲
  slideHeld = null; jumpHeld = null;
  uiTouch.jumpBtn = false; uiTouch.slideBtn = false; uiTouch.slideTouchId = null; uiTouch.slideGestureId = null;
  saveBest();
  stopBGM();
});
...
wx.onTouchStart(function(e){
  ...
  const wasSlideBtn = uiTouch.slideBtn;   // 【审查修正】只在"这一下真按中滑钮"(false→true 跳变)时登记手指——
  UI.touchStart(t.clientX * DPR, t.clientY * DPR);   //   否则滑钮按住期间任何新手指按下都会把配对抢走，蹲会被误松/粘住
  if(!wasSlideBtn && uiTouch.slideBtn) uiTouch.slideTouchId = activeTouchId;
});
...
function touchUp(e){
  // 【长按蹲】抬起的手指里若有"按住滑钮"的那根：先松蹲再走常规流程（多指玩法：一指按住蹲、另一指点跳，
  //   跳的手指成了"正在操作的手指"，蹲的手指抬起会被下面那行拦掉，蹲就永远松不开——所以要在拦截前单独处理）
  if(e && e.changedTouches && uiTouch.slideTouchId !== null && touchOf(e.changedTouches, uiTouch.slideTouchId)){
    uiTouch.slideTouchId = null; uiTouch.slideBtn = false; releaseSlide('btn');
  }
  // 【审查修正】手势蹲同样按手指配对：滑出蹲的那根手指抬起才松，别的手指抬起不打断
  if(e && e.changedTouches && uiTouch.slideGestureId !== null && touchOf(e.changedTouches, uiTouch.slideGestureId)){
    uiTouch.slideGestureId = null; releaseSlide('pointer');
  }
  // 【审查修正】极端兜底：拿不到抬起明细的取消事件（来电等）——所有长按一律视为松开，宁可站起也不卡蹲
  if(!e || !e.changedTouches || !e.changedTouches.length){
    uiTouch.slideTouchId = null; uiTouch.slideBtn = false; uiTouch.slideGestureId = null;
    releaseSlide('btn'); releaseSlide('pointer');
  }
...
const sfx = {
  ...
  gear(){ beep({f0:600, f1:1400, dur:0.16, type:'sine', vol:0.06}); },      // 【节奏大修】提速预告：一声上扬的"嗖——"（配 ⚡提速 横幅）
  ...
  record(){   // 【界面手感】破纪录小凯歌：赢了就不该放丧乐
    ...
    beep({f0:659,  dur:0.09, type:'square',   vol:0.05, delay:0.09});
    beep({f0:784,  dur:0.09, type:'square',   vol:0.05, delay:0.18});
    beep({f0:1047, dur:0.28, type:'square',   vol:0.055, delay:0.27});
    beep({f0:784,  dur:0.12, type:'triangle', vol:0.05, delay:0.62});
    beep({f0:1047, dur:0.5,  type:'triangle', vol:0.06, delay:0.74});
  },
...
const face = { mood: '', until: 0 };   // mood: 'hurt'=痛 | 'joy'=开心 | 'focus'=专注(【酷跑1】下滑) | 'proud'=得意(连击狂热) | 'fear'=害怕(血少/巨石逼近) | ''=平常
...
let bossAt = BOSS_SCORE;    // 【血条Boss】下一只 Boss 的触发分数（击败后 +15000，中后期循环出现）
...
let lastSpeedLv = 0;        // 【节奏大修】已预告过的最高速度档位（横幅去重；startGame 清零）
let gearFxUntil = 0;        // 【节奏大修】提速演出窗口：预告+渐变期间速度线加浓（只动局部特效，不晃屏不红闪）
...
function showBanner(text, dur, color){
  ...
  banner.homeBorn = homeOpen();   // 【修】记下这条横幅是不是在主页弹的（主页只画主页出生的）
}
...
function renderShareCard(){
  const c = shctx;
  if(shareArtImg.complete && shareArtImg.width > 0){
    // 【美术】新版战报图：AI 主视觉打底（1000×800 → 500×400 正好 5:4），左上清空区盖 Logo，底部暗纱压分数
    c.drawImage(shareArtImg, 0, 0, 500, 400);
    if(logoImg.complete && logoImg.width > 0){
      const lw = 165, lh = lw * (logoImg.height / logoImg.width);
      c.drawImage(logoImg, 18, 14, lw, lh);
    }
    const sg = c.createLinearGradient(0, 235, 0, 400);   // 底部暗纱：亮图上压出可读的文字区
    sg.addColorStop(0, 'rgba(20,12,4,0)'); sg.addColorStop(0.45, 'rgba(20,12,4,0.55)'); sg.addColorStop(1, 'rgba(20,12,4,0.8)');
    c.fillStyle = sg; c.fillRect(0, 235, 500, 165);
    const nick = save.nick || '神秘小狐狸';
    c.textAlign = 'center'; c.textBaseline = 'middle'; c.lineJoin = 'round';
    c.font = 'bold 20px ' + FONT; c.fillStyle = '#ffe9b0';
    c.fillText(playerTitle() + nick, 250, 290);
    c.font = 'bold 44px ' + FONT;
    c.strokeStyle = 'rgba(60,30,0,0.85)'; c.lineWidth = 6;
    c.strokeText('最高 ' + game.best + ' 分', 250, 333);
    c.fillStyle = '#ffd34d';
    c.fillText('最高 ' + game.best + ' 分', 250, 333);
    c.font = 'bold 17px ' + FONT; c.fillStyle = '#ffffff';
    c.fillText('敢来超过我吗？微信搜「狐狸快跑呀」', 250, 374);
    return;
  }
  // ——（回退）图没加载出来：沿用原来的纯代码画法 ——
...
function addCombo(){
  ...
  if(combo > 0 && combo % feverNeed === 0){   // 每攒满 feverNeed 连击触发一次狂热
    ...
    addStat('fevers', 1);
    juiceVibrate('fever');
    setFace('proud', 1.3);   // 【灵动】得意脸：连击攒满的这一下，本狐可是职业的
  }
}
...
function juiceVibrate(kind){
  if(muted) return;   // 【审查修正】静音=连震动一起静默——不然玩家关不掉震动（暂无独立震动开关）
...
const player = {
  ...
  tailLag: 0,                // 【灵动】尾巴延迟跟随角：慢半拍追"空中上翘"的目标角，起跳落地甩出鞭感
  dustT: 0,                  // 跑步扬尘计时
...
function resetPlayer(){
  ...
  player.deadSpin = 0; player.deadSettled = false; player.deadSettleTarget = 0; player.deadSettledAt = 0;   // 【死亡小剧场】重开时站起来
  player.tailLag = 0;   // 【灵动】尾巴延迟角复位（重开/复活要清动画状态的铁律）
  ...
  slideHeld = null;   // 【长按蹲】重开时视为没人按着，上一局按住的手不带进新局
}
...
let jumpHeld = null;
// 【长按蹲】下滑现在被谁按着：null=没人 | 'key'=键盘↓/S | 'btn'=屏幕滑钮 | 'pointer'=向下滑手势。
// 按住不松 = 一直保持蹲滑；和 jumpHeld 一样，按下/松开必须同一方配对，互不干扰。
let slideHeld = null;
function pressJump(source){
  ...
  jumpHeld = source;
  if(player.sliding){   // 【酷跑1】滑行中不起跳（跳+滑是独立动作）；【审查修正】但记入跳跃缓冲——松蹲一瞬自动兑现，不无声吞掉输入
...
function airJump(){
  ...
  juiceVibrate('jump');   // 【触感】空中连跳也轻震一下
...
function startSlide(source, resume){
  if(game.state !== 'playing' || paused || loadingStart) return;
  if(source) slideHeld = source;   // 【长按蹲】记住是谁按住的：没松开就一直蹲，空中按住落地还会自动续蹲
  const p = player;
  ...
  if(face.mood === 'focus' || bgTime > face.until) setFace('focus', slideDur);   // 专注表情（审查修正：别盖掉受击等更重要的表情）
  if(resume){   // 【长按蹲·审查修正】落地自动续蹲只轻扬一点灰：音效/震动/大扬尘不重放，连跳几个坡不该"唰唰唰"轰炸
    puff(p.x + 4, GROUND_Y - 2);
    return;
  }
  // 扬尘：脚后跟两团灰，像在地上蹭出一道
  ...
  sfx.slide();   // 一个低沉的"唰"
  juiceVibrate('slide');   // 【触感】入铲瞬间指尖轻轻"哒"一下
}
// 【长按蹲】按住下滑的那一方松开了才允许站起（键盘/滑钮/手势互不干扰，同 releaseJump 的配对逻辑）
function releaseSlide(source){
  if(slideHeld !== source) return;
  slideHeld = null;
}
/* ========== 8. 开始 / 死亡 / 重开 ========== */
function startGame(){
  ...
  lastSpeedLv = 0; gearFxUntil = 0;   // 【节奏大修】提速档位预告从头来
...
function die(cause){
  ...
  // 【界面手感】破纪录那局是喜事：奏小凯歌，不放"游戏结束"丧乐（把进步当失败处理是大忌）
  if(game.newBest && endlessOnly()) sfx.record(); else sfx.die();
...
function stumble(otype){
  if(otype) lastHitType = otype;   // 【界面手感】记下凶手——结算卡上告诉玩家"是谁干的"
...
function chaseHit(){
  ...
  playerHP -= 30; lastHurtAt = bgTime; invulnUntil = bgTime + 1.0; lastHitType = 'chaser';
...
function revive(){
  ...
  player.sliding = false; player.h = 36; player.squash = 0; player.squashV = 0; player.flipT = -9; player.tailLag = 0;
...
function petRevive(){
  player.x = 120; player.y = GROUND_Y; player.vy = 0; player.grounded = true;
  player.inPit = false; player.jumpsUsed = 0; player.gliding = false;
  player.sliding = false; player.slideUntil = 0; player.h = 36; player.tailLag = 0;   // 顺手收掉下滑状态，避免复活时压矮
...
function bossHurt(dmg){
  ...
  playerHP -= dmg;
  lastHurtAt = bgTime; lastHitType = 'boss';
...
function defeatBoss(){
  bossDefeated = true;
  bossAt = game.score + 15000;   // 【血条Boss】下一只 Boss 再过 15000 分出现（【节奏大修】18000→15000 配合降速补偿，循环间隔的真实时间基本不变）
  boss = null;
  ...
  ringFx(player.x + player.w / 2, player.y - player.h / 2, '#ffd34d');   // 【爽感】屠龙冲击波
...
/* ========== 9. 粒子特效 ========== */
// 【爽感】粒子分形态：以前所有爆花都是同款小方块，现在按事件分形状——
//   circle=柔光圆点 / star=四角小星 / spark=沿飞行方向拉长的火花条 / ring=扩散冲击波
//   kind 不传 = 圆点+火花随机混合（比清一色方块生动）；'coin' = 圆点+星星混合并带发光叠加
function burst(x, y, n, colors, kind){
  for(let i = 0; i < n; i++){
    let shape = kind, glow = false;
    if(kind === 'coin'){ shape = Math.random() < 0.6 ? 'circle' : 'star'; glow = true; }
    else if(!kind){ shape = Math.random() < 0.55 ? 'circle' : 'spark'; }
    particles.push({
      x: x, y: y,
      ...
      grav: 600,
      shape: shape, glow: glow,
    });
  }
}
// 【爽感】冲击波圆环：一圈快速扩散变淡的光环，专用于"大事发生"（开箱/破纪录/屠龙/集齐/复活）
function ringFx(x, y, color){
  particles.push({ x: x, y: y, vx: 0, vy: 0, life: 0, max: 0.5, size: 12, color: color, grav: 0, shape: 'ring', glow: true });
}
...
// 【爽感】飞币：吃到的金币变成一枚小金光沿弧线飞向左上角计数器，到账瞬间计数器弹一下——
//   "钱真的进账了"的视觉闭环（天天酷跑标配）。逻辑上钱是立刻入账的，飞的只是表现。
const flyCoins = [];
// 【性能】低配机降级开关：跳过发光混合/渐变飞币/金币拖影/速度线，只留核心反馈。
//   微信版启动时按 benchmarkLevel 判定；两版运行中持续掉帧也会自动拉高。
let lowFx = false, slowFrames = 0;
let coinPopT = -9;    // 计数器最近一次"弹跳"的时刻（飞币到达时刷新）
let comboPopT = -9;   // 连击数字最近一次弹跳的时刻（每次连击+1时刷新）
function updateParticles(dt){
  ...
  // 飞币推进：0.38 秒飞到计数器，到账触发计数器弹跳
  for(let i = flyCoins.length - 1; i >= 0; i--){
    flyCoins[i].t += dt;
    if(flyCoins[i].t >= 0.38){ coinPopT = bgTime; flyCoins.splice(i, 1); }
  }
}
...
function updateTrail(dt){
  if(game.state === 'playing'){
    ...
    // 【爽感】狂热时刻：尾迹强制换成火焰暖色、更大更密——高潮状态肉眼可见地"燃"起来
    const fever = bgTime < feverUntil;
    const tc = fever ? ['#ff8a5c', '#ffd34d', '#fff6d8'] : (cur.trail || ['#ffd34d']);
    if(bgTime - trailEmitT > 0.028){
      trailEmitT = bgTime;
      let n = (boostDist > 0 && game.runDist < boostDist) ? 2 : 1;   // 冲刺时尾迹更浓
      if(fever) n += 2;                                              // 狂热：喷得更猛
      for(let i = 0; i < n; i++){
        charTrail.push({
          x: player.x + rand(-2, 8), y: player.y - player.h * 0.5 + rand(-7, 5),
          vx: -90 - rand(0, 60), vy: rand(-14, 14),
          life: 0, max: rand(0.35, 0.55), size: fever ? rand(7, 12) : rand(5, 9),
...
const PATTERNS = [
  ...
  // 【丰富度】tier4 硬核段（5200米后才放，【节奏大修】与新速度曲线 640-660 档对齐）：逼你在零点几秒里连续切换跳/滑，拉开高手差距、给老手"手指打结也超爽"的段
...
// 【可玩性】把里程映射成连续的"丰富度档位"(0..4)，锚点 0/280/1100/2200/5200 米线性插值——配合下面 frac² 概率渐入，让障碍花样平滑变多，而不是到某米数整档突然全换（【节奏大修】硬核档 4500→5200 米：与新速度曲线的 640-660 档对齐）
function tierFloat(d){
  const pts = [[0, 0], [280, 1], [1100, 2], [2200, 3], [5200, 4]];
  if(d >= 5200) return 4;
...
function spawnObstacle(){
  ...
  if(!patQueue.length){
    ...
    const tf = tierFloat(d);
    let tier = Math.floor(tf);
    if(tier < 4 && srand() < (tf - tier) * (tf - tier)) tier++;   // 【可玩性】软过渡：越接近下一档边界，越常掺入高一档的段（frac² 让早期只偶尔预览、不突兀拔高），到点不再障碍整档突然全换/变难。档位锚点：280米滑铲/坑/蹦床、1100米火焰/锯/激光、2200米更密、5200米硬核
  ...
  // 【节奏大修】障碍密度曲线：开局稀疏热身，越远越密（触底 0.55 从 3150 米推迟到 4500 米——降速后后期难度改由内容承担，不靠挤间距）；
  // 呼吸波 ±12% 让紧张段/喘息段交替（sin(d/300) 真周期=2π×300≈1885 米、顶速下约 35 秒一轮，别当成"300 米一轮"去改）。
  // 外层再夹一次 0.55：呼吸波只往"更松"方向突破（喘息段最稀 ~0.62），波谷穿不透铁底——任何里程的最坏间距都不比旧版紧（审查修正：外层 max 别删，删了波谷=0.484 比旧版地狱段还挤）
  const densK = (d < 300 ? 1.25 : 1) * Math.max(0.55, Math.max(0.55, 1 - d / 10000) * (1 + 0.12 * Math.sin(d / 300)));
...
// 【爽感】道具激活闪光：吃到任何道具的瞬间，角色身上绽放两圈扩散光环（"变身"的高光帧）
let powerFlashT = -9, powerFlashColor = '#ffd34d';
// 【爽感】变大/缩小的体型补间：不再一帧瞬变，而是快速滑到目标大小（碰撞框照旧瞬切，只有画面在过渡）
let gsCur = 1, shrinkCur = 1;
function activatePower(type){
  powerFlashT = bgTime;
  powerFlashColor = (POWER_INFO[type] && POWER_INFO[type].color) || '#ffd34d';
  ...
  setFace(type === 'dash' ? 'focus' : 'joy', type === 'dash' ? 2.5 : 1.2);   // 【灵动】冲刺是"用力"不是"捡到糖"：专注脸配残影
  sfx.power();
...
function update(dt){
    ...
    if(game.deathBy === 'hit' && !p.deadSettled){
      ...
      if(p.y >= GROUND_Y && p.vy > 0 && !p.inPit && !overPit()){   // 摔回地面（脚下得真有地，别悬在坑口）
        ...
        if(p.vy > 150){
          ...
          p.vy = 0; p.deadSettled = true; p.deadSettledAt = bgTime;  // 没劲了：准备躺平（记下时刻，躺平后还要蹬两下腿+咽气）
  ...
  // 【节奏大修】速度档位制：查表拿当前档目标速度，档间差值固定 2.5 秒爬完（平滑无瞬跳）；到位后停在平台期，玩家有时间适应
  const gearMi = game.runDist / 12;
  const gearNo = gearIdx(gearMi);
  const gearUp = gearNo > 0 ? (GEARS[gearNo][1] - GEARS[gearNo - 1][1]) / 2.5 : 0;
  game.speed = Math.min(GEARS[gearNo][1], game.speed + gearUp * dt);
  // 提速预告：距下一档还差约 1.5 秒路程时横幅+上扬音效打招呼——"变快"成为能期待的进阶而不是失控（纯里程判定，不耗 srand，同种子同图）
  // （审查修正）占线让路：横幅是共享单槽，新手教学/坑警示/Boss开场/奖励关开场正在展示时不抢（bgTime>=banner.until）；Boss战和奖励关期间也不发——速度照常爬档只是不打招呼，错过的档位号下一档自动跳号
  if(gearNo + 1 < GEARS.length && gearNo + 1 > lastSpeedLv && bgTime >= banner.until && !bossMode && bgTime >= bonusUntil && gearMi >= GEARS[gearNo + 1][0] - 1.5 * game.speed / 12){
    lastSpeedLv = gearNo + 1;
    showBanner('⚡ 提速 Lv.' + lastSpeedLv + '！', 1.6, '#ffd34d');
    sfx.gear();
    gearFxUntil = bgTime + 4;   // 预告 1.5 秒 + 渐变 2.5 秒：这段时间速度线加浓（局部特效）
  }
  ...
  // 【灵动】害怕表情：血快见底 或 巨石追到屁股后头，角色自己也慌——只在没别的表情时补位，不抢戏
  if(bgTime >= face.until && (playerHP <= effMaxHP() * 0.25 || (bgTime < chaseUntil && chaseX > player.x - 150)))
    setFace('fear', 0.4);
    ...
    for(const o of obstacles){
      ...
      for(const ob of boxes){
        if(pb.x < ob.x + ob.w && pb.x + pb.w > ob.x &&
          ...
          stumble(o.type);                 // 休闲模式：绊一下，继续跑（顺手记下死因）
          break;
        }
      }
  ...
  if(bgTime < magnetUntil || power.type === 'dash' || power.type === 'fly' || bgTime < petPulseUntil || magnetCat){
    ...
    for(const c of coins){
      ...
      if(dd < mRad && dd > 1){
        c.x += dx / dd * mPull * dt;
        c.y += dy / dd * mPull * dt;
        // 【爽感】记下"正在被吸"的方向和时刻：drawCoins 会画出一截金色拖影（金币划着光尾飞过来）
        c.pull = bgTime; c.mvx = dx / dd * mPull; c.mvy = dy / dd * mPull;
      }
    }
  ...
  for(let i = coins.length - 1; i >= 0; i--){
    ...
    if((c.x - nx) * (c.x - nx) + (c.y - ny) * (c.y - ny) < cr * cr){
      ...
      if(c.star){   // 奖励关的"大头"：直接加表现分，不算金币（所以不会再连环触发奖励关）
        ...
        sfx.coin();
        burst(c.x, c.y, 8, ['#ffd34d', '#ffffff'], 'coin');
      ...
      sfx.coin();
      burst(c.x, c.y, 6, ['#ffd34d', '#fff3b0'], 'coin');
      // 【爽感】一枚小金光沿弧线飞向左上角金币计数器（太密时不叠加，防止磁铁风暴刷爆画面）
      if(flyCoins.length < 24) flyCoins.push({ x: c.x, y: c.y, t: 0 });
...
function updatePlayer(dt){
  ...
  // 【长按蹲】还按着下滑就不断顺延"站起时刻"——按多久蹲多久（轻点一下仍按老规矩滑满一次 SLIDE_DUR）
  if(p.sliding && slideHeld && p.slideUntil < bgTime + 0.05){
    p.slideUntil = bgTime + 0.05;
    if(face.mood === 'focus' || bgTime > face.until) setFace('focus', 0.12);   // 专注脸跟着保持，但别盖掉受击等更重要的表情
  }
  if(p.sliding && (bgTime > p.slideUntil || !p.grounded)) endSlide();
  // 【长按蹲】手还按着但人是站着的（被起跳/踩空/蹦床打断过）：一踩实地面就自动续蹲，不用再按一次
  if(!p.sliding && slideHeld && p.grounded && !p.inPit) startSlide(slideHeld, true);   // resume=true：不重放入铲演出
  ...
  const canJump = !p.sliding && (p.grounded || (bgTime - p.lastGrounded <= COYOTE && p.vy >= 0));   // 滑行中缓冲不兑现，站起那帧才起跳
  if(bgTime - p.lastPress <= BUFFER && canJump){
    ...
    sfx.jump();
    juiceVibrate('jump');   // 【触感】蹬地瞬间指尖轻轻一震
    puff(p.x + 6, GROUND_Y - 2); puff(p.x + p.w - 6, GROUND_Y - 2);
  }
  ...
  if(!p.grounded){
    ...
    if(p.y >= GROUND_Y && !p.inPit && !overPit()){   // 落地（脚下得有地，且不是已坠坑状态）
      const landV = p.vy;   // 【灵动】落地分级要用触地一瞬的下落速度，先存一份再清零
      p.y = GROUND_Y; p.vy = 0; p.grounded = true;
      p.jumpsUsed = 0; p.gliding = false;   // 落地后连跳次数重置
      // 【灵动】落地冲击分级：小落轻压、大落狠砸（正常满跳落速≈820→0.31，和老值0.3持平；上限0.42防出格）
      p.squash = clamp(0.10 + landV / 4000, 0.12, 0.42); p.squashV = 0;   // 弹簧会自己弹回来还超一点，Q弹
      sfx.land();
      puff(p.x + 10, GROUND_Y - 2); puff(p.x + p.w - 10, GROUND_Y - 2);
      if(landV > 1000) burst(p.x + p.w / 2, GROUND_Y - 2, 10, ['#e6dcc2', '#c9bfa0', '#ffffff']);   // 大落差砸地：多扬一圈尘
...
function tickPlayerCosmetics(dt){
  ...
  p.sy = 1 - p.squash;
  p.sx = 1 + p.squash * 0.7;
  // 【爽感】变大/缩小体型补间：快速但不瞬间地滑向目标大小，"变身"有过程感
  const gsT = (power.type === 'giant') ? 1.6 : 1;
  gsCur += (gsT - gsCur) * Math.min(1, dt * 9);
  const shT = (game.state === 'playing' && bgTime < shrinkUntil) ? 0.6 : 1;
  shrinkCur += (shT - shrinkCur) * Math.min(1, dt * 9);
  if(p.blinking > 0) p.blinking -= dt;
  else {
    p.blinkT -= dt;
    if(p.blinkT <= 0){ p.blinking = 0.12; p.blinkT = rand(1.5, 4); }
  }
  // 【灵动】尾巴延迟跟随：目标=着地放平/空中上翘（下落时随落速再抬一点），尾巴慢半拍追过去。
  //   这份"慢半拍"就是次级运动的鞭感——身体先动、尾巴后甩，起跳落地一下就活了
  const tailGoal = p.grounded ? 0 : 0.45 + clamp(p.vy / 3600, -0.18, 0.2);
  p.tailLag += (tailGoal - p.tailLag) * Math.min(1, dt * 8);
}
...
function drawCoins(){
  for(const c of coins){
    ...
    // 【爽感】被磁铁吸着飞的金币：身后拖一截渐淡的金色光尾（速度感）
    if(!lowFx && c.pull && bgTime - c.pull < 0.06){
      ctx.save();
      ctx.globalCompositeOperation = 'lighter';
      const tg = ctx.createLinearGradient(c.x - c.mvx * 0.09, c.y - c.mvy * 0.09, c.x, c.y);
      tg.addColorStop(0, 'rgba(255,211,77,0)');
      tg.addColorStop(1, 'rgba(255,211,77,0.5)');
      ctx.strokeStyle = tg; ctx.lineWidth = 5; ctx.lineCap = 'round';
      ctx.beginPath();
      ctx.moveTo(c.x - c.mvx * 0.09, c.y - c.mvy * 0.09);
      ctx.lineTo(c.x, c.y);
      ctx.stroke();
      ctx.restore();
    }
...
function drawPlayer(){
  ...
  // 【爽感】收益类 buff 的持续标识：双倍金币=金圈、分数狂潮=粉圈，脚下呼吸光——一眼看出"buff 还开着"
  if(game.state === 'playing'){
    let gi2 = 0;
    const glowList = [];
    if(bgTime < coinx2Until) glowList.push('255,211,77');
    if(bgTime < scorex3Until) glowList.push('255,179,246');
    for(const col of glowList){
      const aa = 0.20 + 0.08 * Math.sin(bgTime * 5 + gi2 * 2.1);
      ctx.save();
      ctx.translate(p.x + p.w / 2, p.y - 2);
      ctx.scale(1, 0.32);   // 圆压成椭圆贴地
      const rg = ctx.createRadialGradient(0, 0, 4, 0, 0, 38 + gi2 * 9);
      rg.addColorStop(0, 'rgba(' + col + ',' + aa.toFixed(3) + ')');
      rg.addColorStop(1, 'rgba(' + col + ',0)');
      ctx.fillStyle = rg;
      ctx.beginPath(); ctx.arc(0, 0, 38 + gi2 * 9, 0, TAU); ctx.fill();
      ctx.restore();
      gi2++;
    }
  }
  ...
  const ghostReal = game.state === 'playing' && bgTime < ghostUntil;
  // 【灵动】残影三来源：幽灵道具 / 冲刺(dash) / 二段跳空翻窗口——"快"和"帅"焊在角色自己身上（低配机只留幽灵的）
  const ghosting = ghostReal || (!lowFx && game.state === 'playing' && (power.type === 'dash' || bgTime - p.flipT < 0.45));
  const shrinkS = shrinkCur;   // 缩小药水：身形缩到 0.6（补间过渡，和碰撞框缩小相呼应）
  // 【灵动】速度耦合体态：跑得越快越前倾、颠簸越大、围巾被吹得越平——档位成长直接写在姿态上
  const lean = game.state === 'playing' ? clamp((game.speed - 300) / 500, 0, 1) : 0;
  // 【灵动】软耳朵角度：落地压扁耳朵下折、起跳拉长后掠、跑步随步伐小颤（全借现成弹簧/相位值，零新状态）
  const earA = clamp(-p.squash * 0.6 + (p.grounded && game.state === 'playing' ? Math.sin(p.phase) * 0.05 : 0) +
                     (!p.grounded ? clamp(p.vy / 4000, -0.1, 0.12) : 0), -0.45, 0.45);
  // 【灵动】躺平后 0.9 秒内蹬两下腿再静止（漫画式收尾；只抖腿，躯干角度=π 的铁律不碰）
  const deadKick = (game.state === 'dead' && game.deathBy === 'hit' && p.deadSettled && p.deadSettleTarget &&
                    p.deadSettledAt && bgTime - p.deadSettledAt < 0.9)
    ? Math.sin(bgTime * 18) * (1 - (bgTime - p.deadSettledAt) / 0.9) : 0;
  // 【动画】本体和幽灵残影共用的姿态（算一次，两边都用，保证残影跟本体真"同款"）
  const bobY = (p.grounded && !p.sliding && game.state !== 'dead') ? -Math.abs(Math.sin(p.phase)) * (3 + lean * 1.5) : 0;   // 跑步颠簸（高速颠得更欢）
  const slideY = p.sliding ? 0.58 : 1, slideX = p.sliding ? 1.1 : 1;   // 【酷跑1】下滑压低（形变减轻，滑铲感靠腿部姿势撑）
  const poseRotate = () => {   // 滑铲前倾 / 滑翔 / 二段跳空翻 / 随抛物线俯仰 /【灵动】高速前倾
    if(p.sliding) ctx.rotate(0.35);
      ...
      else ctx.rotate(clamp(p.vy / 1600, -0.15, 0.18) + lean * 0.04);   // 【动画】上升微仰、下落前倾（高速再多俯一点）
    }
    else if(game.state === 'playing' && p.grounded) ctx.rotate(lean * 0.10 + (power.type === 'dash' ? 0.06 : 0));   // 【灵动】越快越"扑"着冲，冲刺再加一口劲
  };
  if(ghosting){
    for(let gi = 2; gi >= 1; gi--){
      ...
      drawCharacter(ctx, ch, {
        ...
        earA, tailK: p.tailLag, lean,   // 【灵动】耳朵/尾巴/前倾也同款，残影不掉队
  ...
  if(ghostReal) ctx.globalAlpha *= 0.5;   // 【内容扩展】只有真幽灵本体才半透明；冲刺/空翻的残影不影响本体
  // 【动画】跑步颠簸 bobY：脚踏实地跑时身体随步伐上下起伏——"贴纸感"变"跑起来了"的关键一笔
  ctx.translate(p.x + p.w / 2, p.y + bobY);   // 以脚底中心为基准
  const gs = gsCur;   // 变大药水：整只放大（补间过渡，不再一帧瞬变）
  ...
  drawCharacter(ctx, ch, {
    time: bgTime,
    grounded: p.grounded,
    swing: p.grounded ? Math.sin(p.phase) * 0.6 + deadKick * 0.3 : deadKick * 0.3,   // 【灵动】躺平蹬腿也走 swing
    airK: clamp((p.vy + 820) / 1640, 0, 1) + deadKick * 0.18,   // 【动画】空中相位：0=刚起跳(收腿团身)→1=下落(伸腿准备着地)；躺平时腿再抖两下
    sliding: p.sliding,                        // 【动画】滑铲专属腿部姿势
    gliding: p.gliding,
    blinking: p.blinking,
    earA, tailK: p.tailLag, lean,              // 【灵动】软耳朵 / 尾巴延迟跟随 / 高速吹平围巾
  ...
  ctx.restore();
  // 【灵动】躺平后的漫画收尾：一缕小白气从嘴边"咽气"飘走（配合上面的蹬腿，躺平不再瞬间变标本）
  if(game.state === 'dead' && game.deathBy === 'hit' && p.deadSettled && p.deadSettleTarget && p.deadSettledAt){
    const gk = (bgTime - p.deadSettledAt) / 1.4;
    if(gk > 0.15 && gk < 1){
      ctx.save();
      ctx.globalAlpha = 0.55 * (1 - gk);
      ctx.strokeStyle = '#ffffff'; ctx.lineWidth = 2; ctx.lineCap = 'round';
      const gx2 = p.x + p.w / 2 - 10, gy2 = p.y - 30 - gk * 26;
      ctx.beginPath();
      ctx.moveTo(gx2, gy2 + 6);
      ctx.quadraticCurveTo(gx2 - 4, gy2 + 2, gx2, gy2 - 2);
      ctx.quadraticCurveTo(gx2 + 4, gy2 - 6, gx2, gy2 - 9);
      ctx.stroke();
      ctx.restore();
    }
  }
  ...
  // 【爽感】道具激活闪光：0.4 秒内两圈光环从身上扩散出去 + 中心一团亮斑（变身的高光帧）
  if(bgTime - powerFlashT < 0.4 && game.state === 'playing'){
    const fk = (bgTime - powerFlashT) / 0.4;
    const fcx = p.x + p.w / 2, fcy = p.y - p.h / 2;
    ctx.save();
    ctx.globalCompositeOperation = 'lighter';
    ctx.globalAlpha = (1 - fk) * 0.8;
    ctx.strokeStyle = powerFlashColor; ctx.lineWidth = 4 * (1 - fk) + 1;
    ctx.beginPath(); ctx.arc(fcx, fcy, 18 + 85 * fk, 0, TAU); ctx.stroke();
    ctx.globalAlpha = (1 - fk) * 0.5;
    ctx.beginPath(); ctx.arc(fcx, fcy, 8 + 60 * fk, 0, TAU); ctx.stroke();
    const fg = ctx.createRadialGradient(fcx, fcy, 2, fcx, fcy, 42);
    fg.addColorStop(0, powerFlashColor); fg.addColorStop(1, 'rgba(255,255,255,0)');
    ctx.globalAlpha = (1 - fk) * 0.3;
    ctx.fillStyle = fg;
    ctx.beginPath(); ctx.arc(fcx, fcy, 42, 0, TAU); ctx.fill();
    ctx.restore();
  }
  // 【爽感】磁铁生效中：身边两道旋转的青色虚线弧——看得见的"磁力场"
  if(game.state === 'playing' && bgTime < magnetUntil){
    ctx.save();
    ctx.translate(p.x + p.w / 2, p.y - p.h / 2);
    ctx.setLineDash([9, 13]);
    ctx.lineWidth = 2.5;
    ctx.strokeStyle = 'rgba(125,249,255,0.5)';
    ctx.rotate(bgTime * 2.6);
    ctx.beginPath(); ctx.arc(0, 0, 54 + Math.sin(bgTime * 5) * 4, 0, TAU); ctx.stroke();
    ctx.rotate(-bgTime * 5.4);
    ctx.strokeStyle = 'rgba(125,249,255,0.28)';
    ctx.beginPath(); ctx.arc(0, 0, 64, 0.6, 0.6 + TAU * 0.72); ctx.stroke();
    ctx.setLineDash([]);
    ctx.restore();
  }
...
function drawCharacter(c, ch, o){
  ...
  function mouth(mx, my){   // 【灵动】嘴巴：跟着 face.mood 走——团子终于会笑会咧嘴了（画在鼻子下方）
    c.strokeStyle = '#43281a'; c.lineWidth = 1.8; c.lineCap = 'round';
    if(o.dead){                     // 咽气小张嘴
      c.fillStyle = '#43281a';
      c.beginPath(); c.ellipse(mx, my, 2.2, 2.8, 0, 0, TAU); c.fill();
    } else if(o.face === 'joy'){    // 咧嘴大笑：张开的半圆 + 粉舌头
      c.fillStyle = '#7c3a1d';
      c.beginPath(); c.arc(mx, my - 1, 4.2, 0, Math.PI); c.closePath(); c.fill();
      c.fillStyle = '#ff9d9d';
      c.beginPath(); c.ellipse(mx, my + 1.4, 2.4, 1.5, 0, 0, Math.PI); c.fill();
    } else if(o.face === 'hurt'){   // 痛苦波浪嘴
      c.beginPath();
      c.moveTo(mx - 4, my);
      c.quadraticCurveTo(mx - 2, my - 2.5, mx, my);
      c.quadraticCurveTo(mx + 2, my + 2.5, mx + 4, my);
      c.stroke();
    } else if(o.face === 'focus'){  // 抿紧的一字嘴
      c.beginPath(); c.moveTo(mx - 3, my); c.lineTo(mx + 3, my); c.stroke();
    } else if(o.face === 'proud'){  // 单边上扬的坏笑
      c.beginPath(); c.moveTo(mx - 3.5, my); c.quadraticCurveTo(mx + 1, my + 1.5, mx + 4, my - 2.5); c.stroke();
    } else if(o.face === 'fear'){   // 吓得抿成小波浪
      c.beginPath();
      c.moveTo(mx - 3, my); c.quadraticCurveTo(mx - 1, my + 1.5, mx + 1, my);
      c.quadraticCurveTo(mx + 2.5, my - 1, mx + 3.5, my + 0.5);
      c.stroke();
    } else {                        // 平时一条浅浅的微笑小弧
  ...
  // 【灵动】尾巴摆：有 tailK（本体每帧算好的"延迟跟随角"）就用它代替瞬间上翘——起跳落地尾巴慢半拍甩出鞭感；
  //   高速(lean)时摆幅收小（被风拉直）。预览等没传 tailK 的场合走老规矩。
  const wag = Math.sin(t * 9) * 0.25 * (1 - (o.lean || 0) * 0.3) +
              (o.tailK !== undefined ? o.tailK : (o.grounded ? 0 : 0.45));
  // 【灵动】待机"活物包"（商店/主页预览传 idle:true）：呼吸起伏 + 隔几秒歪一下头，角色不再是定格贴纸
  const idleEar = o.idle ? ((t % 4.7) > 4.25 ? Math.sin(t * 34) * 0.22 : 0) : 0;   // 耳朵隔 4.7 秒抖 0.45 秒
  if(o.idle){
    const br = Math.sin(t * 2.2) * 0.022;   // 呼吸：绕脚底原点轻轻起伏（头动脚不动）
    const tk = (t % 7.3) > 6.75 ? Math.sin(((t % 7.3) - 6.75) / 0.55 * Math.PI) * 0.09 : 0;   // 歪头探身，进出各半
    c.save();
    c.rotate(tk);
    c.scale(1 - br * 0.5, 1 + br);
  }
  ...
  if(o.idle) c.restore();   // 【灵动】收掉待机呼吸/歪头的变换
}
...
function drawParticles(){
  for(const p of particles){
    ...
    ctx.globalAlpha = 1 - k;
    if(p.glow && !lowFx) ctx.globalCompositeOperation = 'lighter';   // 发光叠加：亮色粒子真的"亮"（低配机跳过混合模式切换）
    if(p.shape === 'ring'){
      // 扩散冲击波：半径越来越大、线越来越细
      ctx.strokeStyle = p.color;
      ctx.lineWidth = Math.max(1, 5 * (1 - k));
      ctx.beginPath(); ctx.arc(p.x, p.y, p.size + 95 * k, 0, TAU); ctx.stroke();
    } else if(p.shape === 'star'){
      // 四角小星星：边飞边转
      const r = p.size * (1 - k * 0.5) + 1.5;
      ctx.save(); ctx.translate(p.x, p.y); ctx.rotate(p.life * 3);
      ctx.strokeStyle = p.color; ctx.lineWidth = 1.8; ctx.lineCap = 'round';   // 样式包进 save/restore，不泄漏到函数外
      ctx.beginPath();
    ...
    if(p.glow && !lowFx) ctx.globalCompositeOperation = 'source-over';
  }
  // 飘字：弹出登场（前0.15秒从大到正常）+ 深色描边（亮背景上也看得清）
  ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
  ctx.lineJoin = 'round';
  for(const f of floats){
    ctx.globalAlpha = 1 - f.life / 0.8;
    const s = 1 + 0.5 * Math.max(0, 1 - f.life / 0.15);   // 弹出：1.5倍 → 1倍
    ctx.save();
  ...
  ctx.globalAlpha = 1;
  ctx.lineCap = 'butt'; ctx.lineJoin = 'miter';   // 复位线帽/拐角默认值，别把 round 泄漏给后续绘制
}

/* ========== 13. 界面文字 ========== */
function drawHUD(){
  if(game.state === 'ready' && homeOpen()) return;   // 大厅开着时不画 HUD：一屏一焦点
  // 【爽感】飞币：吃到的金币沿弧线飞向左上角计数器（画在 HUD 文字下层）
  if(flyCoins.length){
    ctx.save();
    ctx.globalCompositeOperation = 'lighter';
    const tx2 = 19 + hudInsetL, ty2 = 53;   // 目标 = 左上角面板的金币图标
    for(const fc of flyCoins){
      const k = fc.t / 0.38, kk = k * k;   // 先慢后快，有"被吸走"感
      const cx2 = (fc.x + tx2) / 2, cy2 = Math.min(fc.y, ty2) - 70;   // 弧线控制点（抬高）
      const px3 = (1 - kk) * (1 - kk) * fc.x + 2 * (1 - kk) * kk * cx2 + kk * kk * tx2;
      const py3 = (1 - kk) * (1 - kk) * fc.y + 2 * (1 - kk) * kk * cy2 + kk * kk * ty2;
      const r3 = 6.5 - 2.5 * k;
      if(lowFx){   // 低配机：纯色小圆点，省掉每帧建径向渐变（视觉闭环保留，开销砍掉）
        ctx.fillStyle = '#ffe27a';
        ctx.beginPath(); ctx.arc(px3, py3, r3 * 0.8, 0, TAU); ctx.fill();
        continue;
      }
      const fg2 = ctx.createRadialGradient(px3, py3, 0.5, px3, py3, r3);
      fg2.addColorStop(0, '#fff6c8'); fg2.addColorStop(0.55, '#ffd34d'); fg2.addColorStop(1, 'rgba(255,211,77,0)');
      ctx.fillStyle = fg2;
      ctx.beginPath(); ctx.arc(px3, py3, r3, 0, TAU); ctx.fill();
    }
    ctx.restore();
  }
  ctx.save();
  ...
  // 【HUD大整理·对标天天酷跑】得分/金币/血条/道具条收进"左上角信息面板"（真·贴角），中路只留进度和连击，
  //   不再挤在屏幕中带。面板右缘压在 112 以内：狐狸泳道从 x=120 起，跳多高都钻不到面板底下（沿用"根治遮挡"思路）。
  //   金币/血量图标改矢量手画——部分安卓微信的 canvas 画 emoji 会变空白/豆腐块，这就是"文字不显示"的根子。
  const CX = W / 2;
  const breaking = endlessOnly() && game.startBest > 0 && game.score > game.startBest && game.state !== 'ready';   // 【酷跑2】闯关不显示破纪录金光
  // ===== 左上角信息面板 =====
  const PX = 4 + hudInsetL, PW = 108, LX = PX + 8;   // 面板外框 / 内容左缘（hudInsetL=刘海让位，网页恒0）
  // 先数一遍要画几条道具条，才能定面板高度（背板一次画好，内容盖上去）
  const pbars = [];
  if(power.type) pbars.push([POWER_INFO[power.type].name, POWER_INFO[power.type].color, (power.until - bgTime) / power.total]);
  if(bgTime < magnetUntil) pbars.push(['磁铁', POWER_INFO.magnet.color, (magnetUntil - bgTime) / magnetTotal]);
  if(bgTime < coinx2Until) pbars.push(['双倍', POWER_INFO.coinx2.color, (coinx2Until - bgTime) / coinx2Total]);
  if(bgTime < slowUntil) pbars.push(['时停', POWER_INFO.slow.color, (slowUntil - bgTime) / slowTotal]);
  if(bgTime < shrinkUntil) pbars.push([POWER_INFO.shrink.name, POWER_INFO.shrink.color, (shrinkUntil - bgTime) / shrinkTotal]);
  if(bgTime < ghostUntil) pbars.push([POWER_INFO.ghost.name, POWER_INFO.ghost.color, (ghostUntil - bgTime) / ghostTotal]);
  if(bgTime < scorex3Until) pbars.push(['狂潮×3', POWER_INFO.scorex3.color, (scorex3Until - bgTime) / scorex3Total]);
  {   // 半透明圆角背板：亮色天空上也一眼读清（顺带治"白字看不清"）
    ctx.save(); ctx.shadowBlur = 0;
    ctx.fillStyle = 'rgba(8,12,24,0.34)';
    rr(PX, 4, PW, 76 + pbars.length * 16, 9); ctx.fill();
    ctx.restore();
  }
  ctx.textAlign = 'left'; ctx.textBaseline = 'top';
  ctx.font = 'bold 10px ' + FONT; ctx.fillStyle = breaking ? '#ffd34d' : '#9fb4d8';   // 小标签
  hudText(breaking ? '得分 · 新纪录!' : '得分', LX, 9);
  ctx.font = 'bold 24px ' + FONT; ctx.fillStyle = breaking ? '#ffd34d' : '#fff';      // 得分大数字（破纪录变金）
  hudText(String(game.score), LX, 19);
  {   // 金币行：矢量小金币 + 数字；飞币到账瞬间整行弹一下（放大回落）
    const cy0 = 46;
    const cp = Math.max(0, 1 - (bgTime - coinPopT) / 0.18);
    ctx.save();
    if(cp > 0){
      const cs2 = 1 + 0.35 * cp;
      ctx.translate(LX + 7, cy0 + 7); ctx.scale(cs2, cs2); ctx.translate(-(LX + 7), -(cy0 + 7));
    }
    ctx.save(); ctx.shadowBlur = 0;   // 手画金币：金圆 + 深描边 + 高光弧（不再用 🪙）
    ctx.fillStyle = '#ffd34d'; ctx.beginPath(); ctx.arc(LX + 7, cy0 + 7, 6.5, 0, TAU); ctx.fill();
    ctx.strokeStyle = '#b07714'; ctx.lineWidth = 1.6; ctx.beginPath(); ctx.arc(LX + 7, cy0 + 7, 6.5, 0, TAU); ctx.stroke();
    ctx.strokeStyle = 'rgba(255,255,255,0.85)'; ctx.lineWidth = 1.4;
    ctx.beginPath(); ctx.arc(LX + 7, cy0 + 7, 3.6, -2.4, -1.1); ctx.stroke();
    ctx.restore();
    ctx.font = 'bold 15px ' + FONT; ctx.fillStyle = '#ffd34d';
    hudText(String(save.coins), LX + 18, cy0);
    ctx.restore();
  }
  {   // 血条：矢量小红心 + 短条 + 纯数字（不再用 ❤）
    const hbX = LX + 14, hbY = 66, hbW = 82, hbH = 11;
    const frac = clamp(playerHP / effMaxHP(), 0, 1);
    const hpColor = frac > 0.5 ? '#5bd66a' : frac > 0.25 ? '#ffd34d' : '#ff5a5a';
    ctx.save(); ctx.shadowBlur = 0;
    ctx.fillStyle = '#ff5a6e';   // 小红心 = 两圆 + 一个三角尖
    ctx.beginPath();
    ctx.arc(LX + 4, hbY + 4.4, 3.1, 0, TAU); ctx.arc(LX + 9.4, hbY + 4.4, 3.1, 0, TAU);
    ctx.fill();
    ctx.beginPath();
    ctx.moveTo(LX + 1.2, hbY + 6.2); ctx.lineTo(LX + 6.7, hbY + 12); ctx.lineTo(LX + 12.2, hbY + 6.2);
    ctx.closePath(); ctx.fill();
    ...
    ctx.restore();
    ctx.font = 'bold 10px ' + FONT; ctx.fillStyle = '#fff'; ctx.textAlign = 'center';
    hudText(String(Math.ceil(playerHP)), hbX + hbW / 2, hbY + 1);
    ctx.textAlign = 'left';
  }
  // 道具剩余时间条：名字 + 短条，一行一条接在血条下（全部在背板里）
  let lY = 82;
  for(const pb of pbars){
    ctx.font = 'bold 12px ' + FONT; ctx.fillStyle = '#fff';
    hudText(pb[0], LX, lY);
    const nameW = ctx.measureText(pb[0]).width, bx = LX + nameW + 5, by = lY + 3;
    const barW = Math.max(24, PW - 16 - nameW - 5);   // 条宽自适应名字长短，整行不出面板
    ctx.save(); ctx.shadowBlur = 0;
    ctx.fillStyle = 'rgba(0,0,0,0.35)'; ctx.fillRect(bx, by, barW, 8);
    ctx.fillStyle = pb[1]; ctx.fillRect(bx, by, barW * clamp(pb[2], 0, 1), 8);
    ctx.restore();
    lY += 16;
  }
  // ===== 中栏（屏幕正中，游标 cY） =====
  ...
  hudText(   // 【HUD大整理】中路只留进度信息，emoji 前缀全部去掉（安卓微信 canvas 画 emoji 会变空白）
    adventureMode && curStage
      ? '第' + curStage.id + '关 · 还剩 ' + Math.max(0, curStage.dist - Math.floor(game.runDist / 12)) + ' 米'
      : (dailyMode ? '日赛 ' + Math.min(3000, Math.floor(game.runDist / 12)) + ' / 3000 米'
                   : '最高 ' + game.best),
    CX, cY);
  cY += 21;
  let subLine, subColor;   // 副行：距纪录 / 挑战 / 新纪录 / 闯关金币进度
  if(adventureMode && curStage){
    subLine = '金币 ' + stageCoins + '/' + curStage.goalCoins + ' · ' + (stageHurt ? '受伤✗' : '不受伤✓');
    ...
    subLine = 'VS 距击败 ' + challenge.name + ' 还差 ' + (challenge.score - game.score + 1) + ' 分'; subColor = '#ff8aa0';
  ...
  if(game.state === 'playing' && combo >= 5){   // 连击
    ...
    // 【爽感】每次连击 +1 数字都弹一下（瞬间放大再回落），越连越有节奏感
    const cPop = Math.max(0, 1 - (bgTime - comboPopT) / 0.16);
    const cFs = Math.round(15 * Math.min(1.5, 1 + combo / 100) * (1 + 0.45 * cPop));
...
function render(){
  ...
  // 【爽感】高速/冲刺/狂热时的速度线：几道贴边白色流光快速掠过，营造"风驰电掣"（低透明度不抢戏）
  if(game.state === 'playing' && !paused){
    const spd = game.speed * dashMult() * (bgTime < slowUntil ? 0.55 : 1);   // 时停期间速度线也跟着世界一起慢（口径与世界移动一致）
    const speedK = clamp((spd - 440) / 260, 0, 1) + (power.type === 'dash' ? 0.55 : 0) + (bgTime < feverUntil ? 0.4 : 0) + (bgTime < gearFxUntil ? 0.5 : 0);   // 【节奏大修】锚点适配新顶速 700；提速演出窗内速度线加浓
    if(speedK > 0.08 && !lowFx){
      ctx.save();
      ctx.globalCompositeOperation = 'lighter';
      ctx.strokeStyle = '#ffffff'; ctx.lineCap = 'round';
      for(let i = 0; i < 10; i++){
        const len = 46 + (i * 37 % 50) + 50 * speedK;
        const sx3 = (W + 160) - ((game.dist * 2.4 + i * 233) % (W + 260));
        const sy3 = 14 + (i * 89) % (H - 40) + Math.sin(i * 5.2) * 8;
        ctx.globalAlpha = (0.05 + 0.10 * Math.min(1, speedK)) * (0.6 + 0.4 * Math.sin(i * 2.4));
        ctx.lineWidth = 1.6 + (i % 3) * 0.7;
        ctx.beginPath(); ctx.moveTo(sx3, sy3); ctx.lineTo(sx3 - len, sy3); ctx.stroke();
      }
      ctx.restore();
    }
  }
  // 【爽感】状态氛围光改"四边内发光"：不再全屏铺色把画面蒙灰——中心保持鲜亮，只有屏幕四周一圈氛围
  const edgeGlow = (r2, g2, b2, a2) => {
    const ew = 70;
    const mk = (x0, y0, x1, y1) => {
      const gd = ctx.createLinearGradient(x0, y0, x1, y1);
      gd.addColorStop(0, 'rgba(' + r2 + ',' + g2 + ',' + b2 + ',' + a2.toFixed(3) + ')');
      gd.addColorStop(1, 'rgba(' + r2 + ',' + g2 + ',' + b2 + ',0)');
      return gd;
    };
    ctx.fillStyle = mk(0, 0, 0, ew);       ctx.fillRect(0, 0, W, ew);        // 上
    ctx.fillStyle = mk(0, H, 0, H - ew);   ctx.fillRect(0, H - ew, W, ew);   // 下
    ctx.fillStyle = mk(0, 0, ew, 0);       ctx.fillRect(0, 0, ew, H);        // 左
    ctx.fillStyle = mk(W, 0, W - ew, 0);   ctx.fillRect(W - ew, 0, ew, H);   // 右
  };
  // 【血条Boss】Boss 战：四边暗红危机光，轻微脉动
  if(boss) edgeGlow(150, 0, 0, 0.30 + 0.12 * (0.5 + 0.5 * Math.sin(bgTime * 4)));
  // 超级奖励时间：四边金光
  if(bgTime < bonusUntil) edgeGlow(255, 205, 70, 0.30);
  // 【留存包】① 狂热时刻：四边火焰橙光呼吸
  if(bgTime < feverUntil) edgeGlow(255, 110, 30, 0.30 + 0.10 * Math.sin(bgTime * 5));
  // 【内容扩展】分数狂潮：四边金光一呼一吸（提示"现在分数 ×3"）
  if(bgTime < scorex3Until) edgeGlow(255, 195, 60, 0.16 + 0.12 * (0.5 + 0.5 * Math.sin(bgTime * 6)));
  // 【爽感】时停：四边幽幽淡蓝（以前时停生效画面上完全看不出来）
  if(bgTime < slowUntil) edgeGlow(110, 170, 255, 0.22);
...
function drawHomeScene(){
  ...
  drawCharacter(hctx, CHARS[save.char] || CHARS.fox, {
    ...
    idle: true,   // 【灵动】主页奔跑的小狐狸：呼吸+抖耳+偶尔歪头，开屏第一眼就是活的
    pal: charC(save.char in CHARS ? save.char : 'fox'),
    avatar: (save.useAvatar && avatarImg && avatarImg.complete && avatarImg.naturalWidth > 0) ? avatarImg : null,
  });
  hctx.restore();
...
const LOAD_TIPS = ['提示：长按跳得更高', '提示：紫色高空鸟——千万别跳！', '提示：黑坑会摔死，看到就跳',
                   ...
                   '提示：连跳角色掉坑瞬间还能自救', '提示：高处横杆要下滑躲过',
                   '提示：下滑按住不放＝一直蹲着跑'];   // 【酷跑1】下滑提示 +【长按蹲】玩法提示
...
function uiBtn(o){
  // 【界面手感】按下反馈：手指按住时按钮往下沉一点（像真的被按进去了）
  const zoneY = o.y;   // 【修】热区永远登记在原始位置——下沉只是画面效果，不能把可点区域也搬走
  if(o.id && uiTouch.on && uiTouch.pendId === o.id) o = Object.assign({}, o, { y: o.y + Math.max(1.5, o.h * 0.07) });
  ...
  if(!o.disabled && o.cb){
    if(o.clip) addZoneClipped(o.id, o.x, zoneY, o.w, o.h, o.clip, o.cb);
    else addZone(o.id, o.x, zoneY, o.w, o.h, o.cb);
  }
}
...
function gachaClick(){   // 🎰 幸运抽奖
  ...
  if(roll < 0.06 && unownedChars.length){          // 6% 大奖：直接抽中新角色！
    ...
    showReveal('🎊 稀有角色：' + CHARS[cid].name, '6%的欧皇！已自动出战', 'epic');   // 【修】号角由揭晓时刻独奏，不再连响三声
...
function buyChar(id){   // 买角色
  ...
  showReveal('🦊 新伙伴：' + CHARS[id].name, '已自动出战！', 'epic', '🎉');   // 【界面手感】买到新角色要庆祝（号角由揭晓时刻独奏）
  saveSave();
}
...
function deathCause(){
  if(game.deathBy !== 'hit') return '';
  const n = OB_NAMES[lastHitType];
  return n ? ('被' + n + '耗光了血，下次躲开它！') : '撞太多次没血了，稳一点！';
}
...
function updateDeadCard(){
  ...
  if(adventureMode && curStage){
    ...
    if(win){
      ...
      deadCard.title = '挑战失败';
      deadCard.sub = game.deathBy === 'pit' ? '掉进坑里啦，看到坑要跳～' : (deathCause() || '撞到障碍没血了，再试一次！');
  ...
  deadCard.title = finish ? '🏁 完赛！' : (game.newBest && endlessOnly() ? '🎉 新纪录诞生！' : '游戏结束');
  ...
  deadCard.sub =
    ...
    (game.newBest && endlessOnly() ? '比之前的最高纪录多 ' + Math.max(1, game.score - game.startBest) + ' 分！'
                                   : (game.deathBy === 'pit' ? '掉进坑里啦，看到坑要跳～' : deathCause()));
...
function uiDrawHome(){
  ...
  // —— 顶部正中：大标题（【美术】艺术字 Logo 优先；没加载出来退回渐变金文字画法） ——
  if(logoImg.complete && logoImg.width > 0){
    const titleY = ch * 0.17;
    const lh = ch * 0.27, lw = lh * (logoImg.width / logoImg.height);   // 高度定尺寸（Logo 是两行叠排的方形构图）
    ctx.save();
    ctx.shadowColor = 'rgba(0,0,0,0.45)'; ctx.shadowBlur = ch * 0.028; ctx.shadowOffsetY = ch * 0.008;
    ctx.drawImage(logoImg, cx - lw / 2, titleY - lh * 0.52, lw, lh);
    ctx.restore();
  } else {
  ...
  defs2.forEach((d, i) => {
    ...
    uiBtn({ id: d[0], x: bx2, y: row2Y, w: bw2c, h: row2H, label: d[1], size: fs(0.03),
            bg: 'rgba(13,16,36,0.85)', fg: '#dfe6f5', stroke: 'rgba(255,255,255,0.25)', bold: false, cb: d[2] });
    // 【界面手感】天赋红点：兜里的钱够升任何一条天赋就亮（把"有便宜可占"晒出来）
    if(d[0] === 'homeTalent'){
      let canUp = false;
      try{ canUp = TALENTS.some(t2 => { const pr2 = talentPrice(t2.id); return pr2 !== null && save.coins >= pr2; }); }catch(e){}
      if(canUp){
        ctx.fillStyle = '#ff4d4d';
        ctx.beginPath(); ctx.arc(bx2 + bw2c - ch * 0.008, row2Y + ch * 0.008, ch * 0.013, 0, TAU); ctx.fill();
        ctx.strokeStyle = '#1d2433'; ctx.lineWidth = Math.max(1, ch * 0.004); ctx.stroke();
      }
    }
  });
...
function drawShopRow(row, x, y, w, h, clip, fs){
  ...
  if(row.prev.kind === 'char'){
    ...
    drawCharacter(ctx, CHARS[id] || CHARS.fox, {
      ...
      blinking: (t % 3) < 0.12 ? 1 : 0, dead: false,
      idle: true,   // 【灵动】货架上的角色也呼吸、抖耳朵、歪头——不再是定格贴纸
...
// 【界面手感】主页横幅：领签到/礼包的反馈以前被主页盖住看不见，现在画一条顶部提示胶囊
function uiDrawHomeBanner(){
  if(bgTime >= banner.until || !banner.homeBorn) return;   // 【修】只画"在主页弹出的"横幅——局内旧横幅不许捡漏到主页
  ctx.save();
  ctx.setTransform(1, 0, 0, 1, 0, 0);
  const cw = canvas.width, ch = canvas.height;
  ctx.globalAlpha = Math.min(1, (banner.until - bgTime) / 0.4);
  ctx.font = 'bold ' + Math.round(ch * 0.036) + 'px ' + FONT;
  const tw = ctx.measureText(banner.text).width, pad2 = ch * 0.03;
  const bw3 = tw + pad2 * 2, bh3 = ch * 0.075, bx3 = (cw - bw3) / 2, by3 = ch * 0.12;
  ctx.fillStyle = 'rgba(10,14,28,0.9)';
  dRR(bx3, by3, bw3, bh3, bh3 / 2); ctx.fill();
  ctx.strokeStyle = 'rgba(255,211,77,0.45)'; ctx.lineWidth = Math.max(1, ch * 0.004);
  dRR(bx3, by3, bw3, bh3, bh3 / 2); ctx.stroke();
  ctx.fillStyle = banner.color || '#ffd34d';
  ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
  ctx.fillText(banner.text, cw / 2, by3 + bh3 / 2 + 1);
  ctx.restore();
}
// 【界面手感】开奖/入手演出：宝箱抖 0.7 秒 → 揭晓奖品（epic=金光 rare=紫光）。点一下跳过。
let revealFx = null;
function showReveal(title, sub, rare, emoji){
  revealFx = { t0: performance.now(), title: title, sub: sub || '', rare: rare || '', emoji: emoji || '🎁' };
}
function uiDrawReveal(){
  if(!revealFx) return;
  const el = (performance.now() - revealFx.t0) / 1000;
  if(el > 2.4){ revealFx = null; return; }
  ctx.save();
  ctx.setTransform(1, 0, 0, 1, 0, 0);
  const cw = canvas.width, ch = canvas.height;
  ctx.fillStyle = 'rgba(5,8,18,0.6)';
  ctx.fillRect(0, 0, cw, ch);
  addZone('revealSkip', 0, 0, cw, ch, () => { revealFx = null; });
  ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
  if(el < 0.7){   // 宝箱抖动
    ctx.save();
    ctx.translate(cw / 2, ch / 2);
    ctx.rotate(Math.sin(el * 44) * 0.16);
    ctx.font = Math.round(ch * 0.2) + 'px ' + FONT;
    ctx.fillText(revealFx.emoji, 0, 0);
    ctx.restore();
  } else {        // 揭晓奖品
    const pk = Math.min(1, (el - 0.7) / 0.3);
    const ps = 0.4 + 0.6 * (1 - Math.pow(1 - pk, 3));
    if(revealFx.justPop === undefined){ revealFx.justPop = 1; sfx.power(); }
    ctx.save();
    ctx.translate(cw / 2, ch / 2 - ch * 0.03);
    ctx.scale(ps, ps);
    const col = revealFx.rare === 'epic' ? '#ffd34d' : (revealFx.rare === 'rare' ? '#d9b8ff' : '#ffffff');
    if(revealFx.rare){   // 稀有度光晕
      const rg2 = ctx.createRadialGradient(0, 0, 6, 0, 0, ch * 0.22);
      rg2.addColorStop(0, col); rg2.addColorStop(1, 'rgba(255,255,255,0)');
      ctx.globalAlpha = 0.28; ctx.fillStyle = rg2;
      ctx.beginPath(); ctx.arc(0, 0, ch * 0.22, 0, TAU); ctx.fill();
      ctx.globalAlpha = 1;
    }
    ctx.font = 'bold ' + Math.round(ch * 0.065) + 'px ' + FONT;
    ctx.strokeStyle = 'rgba(0,0,0,0.6)'; ctx.lineWidth = Math.max(2, ch * 0.008); ctx.lineJoin = 'round';
    ctx.strokeText(revealFx.title, 0, 0);
    ctx.fillStyle = col;
    ctx.fillText(revealFx.title, 0, 0);
    if(revealFx.sub){
      ctx.font = Math.round(ch * 0.034) + 'px ' + FONT;
      ctx.fillStyle = '#cfe0ff';
      ctx.fillText(revealFx.sub, 0, ch * 0.07);
    }
    ctx.restore();
  }
  ctx.restore();
...
function uiDrawDead(){
  // 【界面手感】结算卡入场：第一次画的这帧记时刻——驱动"下滑淡入"和分数滚动；破纪录撒彩花
  if(!deadShownAt){
    deadShownAt = performance.now();
    if(game.newBest && endlessOnly()){
      burst(W / 2, H * 0.3, 30, ['#ffd34d', '#ff8aa0', '#7df9ff', '#ffffff'], 'coin');
      ringFx(W / 2, H * 0.35, '#ffd34d');
    }
  }
  const inK = Math.min(1, (performance.now() - deadShownAt) / 280);
  const inE = 1 - Math.pow(1 - inK, 3);   // 缓动：先快后慢
  ctx.save();
  ctx.setTransform(1, 0, 0, 1, 0, 0);
  const cw = canvas.width, ch = canvas.height;
  const fs = k => Math.max(9, Math.round(ch * k));
  ...
  const cardX = (cw - cardW) / 2, cardY = Math.max(ch * 0.02, (ch - cardH) / 2);   // 【修】入场只淡入不位移——位移动画会让按钮热区跟手指错位丢点击
  ctx.globalAlpha = inE;   // 入场：整卡淡入（函数结尾 restore 会复原）
  uiCard(cardX, cardY, cardW, cardH);
  ...
  y += ch * 0.042;
  {   // 【界面手感】分数滚动跳字：0.9 秒从 0 滚到最终分（闯关显示星星不滚）
    let scoreTxt = deadCard.score;
    if(!deadCard.adv){
...
//   跳钮和滑钮都在 touchStart 里"按下即触发"（跳=长按跳更高；滑=按住一直蹲，抬手站起）。
function actionBtn(cx, cy, r, icon, label, active, rgb){
  if(active) r *= 1.06;   // 【触感】按住/生效时微微放大，手指下有"按进去了"的反馈
...
function drawSlideBtn(){
  ...
  actionBtn(cx, cy, r, '⬇', '滑', player.sliding || !!slideHeld, '142,230,255');
  addZone('btnSlide', cx - r, cy - r, r * 2, r * 2, () => {});   // 【长按蹲】滑钮在 touchStart 里按下即触发
  ctx.restore();
}

/* —— UI 入口：触摸分发 + 各界面的画法 —— */
const uiTouch = { on: false, x: 0, y: 0, moved: 0, pendId: null, startY: 0, slideFired: false, velY: 0, lastMoveT: 0,
                  slideBtn: false, slideTouchId: null,   // 【长按蹲】滑钮是否被按住 + 按住它的手指 id（多指时别的手指抬起不打断蹲）
                  slideGestureId: null };                // 【长按蹲】下滑手势的持有手指 id（同上，手势蹲也按手指配对释放）
let scrollInertia = null;   // 【界面手感】列表惯性滚动：{v:像素/秒}，松手后按速度继续滑、指数衰减   // 当前手指位置 + 累计位移 + 待触发按钮 +【酷跑1】手势起点Y/本次手势是否已触发下滑
const UI = {
  pauseIcon: 'pause',
  loadTip: '',
  ...
  touchStart(x, y){   // x/y 是设备像素坐标
    ensureAudio();
    scrollInertia = null;   // 【修】手指按下=接管列表：正在飞的惯性立刻停，不然按停了松手又飞走
    uiTouch.on = true; uiTouch.x = x; uiTouch.y = y;
    uiTouch.velY = 0; uiTouch.lastMoveT = performance.now();   // 【修】速度采样从本手势起点算，防首帧除以0.001爆表
    ...
    for(let i = UI.zones.length - 1; i >= 0; i--){
      ...
      if(x >= z.x && x <= z.x + z.w && y >= z.y && y <= z.y + z.h){
        ...
        // 【长按蹲】滑钮同样"按下即触发"（以前是抬手才滑，慢半拍）：按住不放=一直蹲着，抬手才站起
        if(z.id === 'btnSlide'){ uiTouch.slideBtn = true; startSlide('btn'); return; }
  ...
  touchMove(x, y){   // 手指拖动：目前只有商店货架需要滚 +【酷跑1】游戏中向下滑＝下滑躲避
    {   // 【界面手感】记录竖向滑动速度（松手后给惯性滚动用）
      const nowT = performance.now();
      const mdt = Math.max(0.001, (nowT - (uiTouch.lastMoveT || nowT)) / 1000);
      uiTouch.velY = uiTouch.velY * 0.4 + ((y - uiTouch.y) / mdt) * 0.6;   // 平滑一下，别被单帧抖动带偏
      uiTouch.lastMoveT = nowT;
    }
    ...
    if(uiTouch.on && !uiTouch.slideFired && game.state === 'playing' && uiScreen === 'none' && !paused && !loadingStart &&
      ...
      uiTouch.slideGestureId = activeTouchId;   // 【多指】记住是哪根手指滑出来的蹲，抬起只认它
      startSlide('pointer');   // 【长按蹲】滑下来的手指不抬起＝一直蹲着
    }
  ...
  touchEnd(){
    uiTouch.on = false;
    // 【界面手感】松手时还有滑动速度且正开着列表 → 惯性滚动接棒
    if(Math.abs(uiTouch.velY) > 220 && (shopOpen() || uiScreen === 'ach' || uiScreen === 'talent' || uiScreen === 'adventure')){
      scrollInertia = { v: clamp(uiTouch.velY, -3600, 3600) };
    }
    uiTouch.velY = 0; uiTouch.lastMoveT = 0;
    if(uiTouch.jumpBtn){ uiTouch.jumpBtn = false; releaseJump('pointer'); return; }   // 跳钮抬手：松开（长按越久跳越高）；手势蹲由 touchUp 按手指配对放，这里不代劳
    if(uiTouch.slideBtn){ uiTouch.slideBtn = false; uiTouch.slideTouchId = null; releaseSlide('btn'); return; }   // 【长按蹲】滑钮抬手：站起来
    ...
    if(pid){
      ...
      releaseJump('pointer');   // 手势蹲的释放在 touchUp 里按手指配对完成，这里不再无差别松（审查修正）
    }
    ensureAudio();
    if(game.state === 'playing' || homeOpen()) startBGM();   // 【主页音乐】大厅也有 BGM
  },
...
function frame(t){
  ...
  last = t;
  // 【性能】低配机自动降级：连续约3秒帧耗≥32ms(≤30fps)就把 lowFx 拉高（单向闸，降了不再升）
  if(!lowFx){
    if(dt >= 0.032) slowFrames++; else slowFrames = Math.max(0, slowFrames - 2);
    if(slowFrames > 90) lowFx = true;
  }
  ...
  if(scrollInertia && !uiTouch.on){
    scrollInertia.v *= Math.pow(0.015, dt);
    const dyI = scrollInertia.v * dt;
    if(Math.abs(scrollInertia.v) < 40) scrollInertia = null;
    else if(shopOpen()) shopScroll = clamp(shopScroll + dyI, Math.min(0, shopViewH - shopContentH), 0);
    else if(uiScreen === 'ach') achScroll = clamp(achScroll + dyI, Math.min(0, achViewH - achContentH), 0);
    else if(uiScreen === 'talent') talentScroll = clamp(talentScroll + dyI, Math.min(0, talentViewH - talentContentH), 0);
    else if(uiScreen === 'adventure') advScroll = clamp(advScroll + dyI, Math.min(0, advViewH - advContentH), 0);
    else scrollInertia = null;
  }
  ...
  if(homeOpen()) uiDrawHomeBanner();   // 【修】领奖横幅画在弹窗遮罩之后——签到面板里领奖也看得见
  uiDrawReveal();   // 【界面手感】开奖/入手演出：宝箱抖动→揭晓，永远画在最上层
