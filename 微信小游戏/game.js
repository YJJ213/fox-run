...
function update(dt){
  ...
  // 【内容扩展】神秘宝箱：奖励关里不出；同屏最多一个；按距离间隔生成（每局约 1~3 个）
  if(!inBonus) distToBox -= move;
  if(!inBonus && distToBox <= 0 && boxes.length === 0){
    spawnBox();
  ...
  // 【内容扩展】字母卡：奖励关里不出；没集齐时按距离间隔掉落
  if(!inBonus && letterCount() < 4) distToLetter -= move;
  if(!inBonus && distToLetter <= 0 && letterCount() < 4){
    spawnLetter();
