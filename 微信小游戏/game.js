...
function startGame(){
  ...
  playerHP = PLAYER_MAX_HP; lastHurtAt = -99;   // 【血条Boss】新一局满血、清空受伤时刻
...
function revive(){
  ...
  playerHP = PLAYER_MAX_HP; lastHurtAt = -99;   // 【血条Boss】复活回满血，否则复活瞬间又是 0 血秒死
...
function activatePower(type){
  ...
  if(type === 'shrink'){
    shrinkTotal = POWER_DUR.shrink + save.durLevel * 1.5;
  ...
  if(type === 'ghost'){
    ghostTotal = POWER_DUR.ghost + save.durLevel * 1.5;
  ...
  if(type === 'scorex3'){
    scorex3Total = POWER_DUR.scorex3 + save.durLevel * 1.5;
...
function update(dt){
  ...
  if(bgTime - lastHurtAt > 2) playerHP = Math.min(PLAYER_MAX_HP, playerHP + 7 * dt);
  ...
  for(let i = coins.length - 1; i >= 0; i--){
    ...
    if((c.x - nx) * (c.x - nx) + (c.y - ny) * (c.y - ny) < cr * cr){
      ...
      game.coinCount += v;
      save.coins += v;
      playerHP = Math.min(PLAYER_MAX_HP, playerHP + 1);   // 【血条Boss】吃币微回 1 血，奖励收集
...
function drawPlayer(){
  ...
  ctx.scale(p.sx * gs * shrinkS, p.sy * gs * shrinkS);   // 压扁/拉伸 × 巨大化 × 【内容扩展】缩小
...
function drawHUD(){
  ...
  // 【血条Boss】血条：金币行下方画一条——背景深槽 + 前景按 HP 比例，颜色随血量变（>50%绿/>25%黄/否则红），带描边 ❤ 图标和数字
  {
    ...
    const frac = clamp(playerHP / PLAYER_MAX_HP, 0, 1);
...
function drawHomeScene(){
  ...
  hctx.fillStyle = '#3a2a00'; hctx.fillText('狐狸快跑呀', 163, 27);
  hctx.fillStyle = '#ffd34d'; hctx.fillText('狐狸快跑呀', 160, 24);
...
// 万能按钮：画底、画字、登记点击区域。cant=买不起（灰底但能点，点了提示差多少钱），disabled=完全不可点
function uiBtn(o){
  const r = Math.min(o.h * 0.3, canvas.height * 0.022);
  if(o.alpha !== undefined) ctx.globalAlpha = o.alpha;
  const bgc = (o.cant || o.disabled) ? '#3a4357' : (o.bg || '#ffd34d');
  if(bgc !== 'none'){ ctx.fillStyle = bgc; dRR(o.x, o.y, o.w, o.h, r); ctx.fill(); }
  if(o.stroke && !o.cant && !o.disabled){
    ctx.strokeStyle = o.stroke;
    ctx.lineWidth = Math.max(1, canvas.height * 0.004);
    dRR(o.x, o.y, o.w, o.h, r); ctx.stroke();
  }
  const fgc = (o.cant || o.disabled) ? '#97a1b8' : (o.fg || '#4a3500');
...
function uiCard(x, y, w, h){   // 深色弹窗卡片底（同网页版 #202737 圆角卡）
  ctx.fillStyle = '#202737';
  dRR(x, y, w, h, canvas.height * 0.03); ctx.fill();
}
  ...
  if(shopTab === 'coin'){
    ...
    for(const g2 of GEM_GOODS){
      rows.push({ prev: { kind: 'emoji', v: g2.emoji }, title: g2.name,
        chips: [{ t: '钻石专属', c: '#23bcc9' }], desc: g2.desc,
        btns: [ save[g2.id] ? { id: 'shopGemBuy-' + g2.id, label: '已拥有', disabled: true }
...
function drawShopRow(row, x, y, w, h, clip, fs){
  ...
  for(let i = row.btns.length - 1; i >= 0; i--){
    const b = row.btns[i];
    ctx.font = 'bold ' + fs(0.032) + 'px ' + FONT;
    const bw3 = Math.max(btnH * 1.5, ctx.measureText(b.label).width + btnH * 0.8);
    bx -= bw3;
    uiBtn({ id: b.id, x: bx, y: y + (h - btnH) / 2, w: bw3, h: btnH, label: b.label, size: fs(0.032),
...
try{
  ...
  wx.onShareAppMessage(function(){
    ...
    return { title: '狐狸快跑呀——像素跑酷，来比比谁跑得远！' };
  });
}catch(e){}
