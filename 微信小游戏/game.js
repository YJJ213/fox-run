...
function startSlide(){
  ...
  p.sliding = true;
  p.slideUntil = bgTime + SLIDE_DUR;
  p.h = SLIDE_H;                  // 身高压到一半：碰撞框和画面都变矮
  setFace('focus', SLIDE_DUR);   // 专注表情：低头猫腰冲过去
  // 扬尘：脚后跟两团灰，像在地上蹭出一道
...
function petRevive(){
  ...
  playerHP = PLAYER_MAX_HP; lastHurtAt = -99;
  invulnUntil = bgTime + 2.5;   // 复活保护期
  pits.length = 0;
  for(let i = obstacles.length - 1; i >= 0; i--){
...
function update(dt){
  ...
  if(bgTime < magnetUntil || power.type === 'dash' || power.type === 'fly' || bgTime < petPulseUntil || magnetCat){
    ...
    const mRad = strong ? 170 : 110, mPull = strong ? 520 : 320;
